...
(define (intersection listA listB)
  (make-set(if (null? (or listA listB)) '()
               (if (element? (car listA) listB) (cons (car listA) (intersection (cdr listA) listB)) (intersection (cdr listA) listB)))))
...
(define (subset? setA setB)
  (if (null? setA) #t
      (if (element? (car setA) setB) (subset? (cdr setA) setB) #f)))
...
(define (set-difference listA listB)
  (make-set (if (null? listA) '() (if (element? (car listA) listB) (set-difference (cdr listA) listB)
...
(define (disjoint? setA setB)
  (if (null? (intersection setA setB)) #t #f))
...
(define (insert element lst)
  (make-set (if (element? element lst) lst (cons element lst))))
