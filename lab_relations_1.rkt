...
(define (symmetric? relation)
   (if (set-equal? relation (flip-pairs relation)) #t #f))
...
(define (symmetric-closure relation)
  (if (symmetric? relation) relation
      (make-set(union relation (flip-pairs relation)))))
...
(define (related-to element relation)
   (if (null? relation) '()
       (if (= element (car(car relation))) (append (cdr(car relation))
                                                   (related-to element (cdr relation)))(related-to element (cdr relation)))))
