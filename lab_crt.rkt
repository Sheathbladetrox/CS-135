...
(define (CRT-exists? tuplelist)
  (if (null? tuplelist) #t
      (and (CRT-exists?-helper (cadar tuplelist) (cdr tuplelist)) (CRT-exists? (cdr tuplelist)))))

(define (CRT-exists?-helper tuple tuplelist)
  (if (null? tuplelist) #t
      (if (= (EuclidAlgo tuple (car(cdr(car tuplelist)))) 1)(and #t (CRT-exists?-helper tuple (cdr tuplelist))) #f)))
...
(define (CRT-helper tuplelist m)
  (if (null? tuplelist) 0
      (+ (* (caar tuplelist) (* (/ m (cadar tuplelist)) (mul_inv (/ m (cadar tuplelist)) (cadar tuplelist))))
...
(define (CRT tuplelist)
  (if (equal? (CRT-exists? tuplelist) #f) -1
      (CRT-counter tuplelist (CRT-helper tuplelist (M tuplelist)))))

(define (CRT-counter tuplelist int)
  (if (< int 0) (CRT-counter tuplelist (+ int (M tuplelist)))
      (if (> (quotient int (M tuplelist)) 1)
          (CRT-counter tuplelist (- int (M tuplelist))) int)))
