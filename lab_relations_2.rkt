...
(define (composite  relationOuter relationInner)
  (if (null? relationInner) '()
      (if (null? (related-to (car(cdr(car relationInner))) relationOuter))
          (composite relationOuter (cdr relationInner))
          (cons (list (car(car relationInner))(car (related-to (car(cdr(car relationInner))) relationOuter)))
                (composite relationOuter (cdr relationInner))))))

;; Define power
;; power takes a relation and applies composite to itself k times(c
...
(define (power relation k)
  (if (= k 0) '()
      (if (= k 1) relation (composite relation (power relation (- k 1))))))
...
(define (transitive-closure relation)
  (make-set (append relation (trans-helper relation (length relation)))))
(define (trans-helper relation n)
  (if (= n 0) '()
      (append (power relation n)(trans-helper relation (- n 1)))))
...
(define (transitive? relation)
   (if (set-equal? (transitive-closure relation) relation) #t #f))
...
(define (EQ-relation? relation n)
  (if (and (transitive? relation) (and (symmetric? relation)(reflexive? relation n))) #t #f))
